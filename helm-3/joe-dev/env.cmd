...
if %ERRORLEVEL% == 0 (
    set KUBECONFIG=
    kubectl config set-cluster ucp_test-ucp.lab.capstonec.net:6443_joe-dev --server https://test-ucp.lab.capstonec.net:6443 --certificate-authority "%~dp0ca.pem" --embed-certs
    kubectl config set-credentials ucp_test-ucp.lab.capstonec.net:6443_joe-dev --client-key "%~dp0key.pem" --client-certificate "%~dp0cert.pem" --embed-certs
